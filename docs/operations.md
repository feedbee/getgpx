...
The backend writes newline-delimited JSON logs to stdout with Pino. `LOG_LEVEL` controls the minimum severity (`info` by default; `debug` for temporary diagnosis). HTTP logs contain a generated `requestId`, method, route template, status, and response time; health request logs are omitted. Unexpected errors and track-analysis failures produce separate events. Request URLs, headers, GPX contents, and OAuth or session values must not be added to log fields. Collect, retain, and search stdout logs outside the application container.
