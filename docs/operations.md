...
The backend writes newline-delimited JSON logs to stdout with Pino, including when its API middleware runs inside Vite via `npm run dev`. `LOG_LEVEL` controls the minimum severity (`info` by default; `debug` for temporary diagnosis). HTTP logs contain a generated `requestId`, method, pathname without query parameters, route template when available, status, and `durationMs` in milliseconds; health request logs are omitted. Unexpected errors and track-analysis failures produce separate events. Query parameters, headers, GPX contents, and OAuth or session values must not be added to log fields. Collect, retain, and search stdout logs outside the application container.
